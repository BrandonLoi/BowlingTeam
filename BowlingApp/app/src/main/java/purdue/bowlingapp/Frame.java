...
public class Frame {
    ...
    public Frame(char firstThrow, char secondThrow) {
        int first = validateInput(firstThrow);
        int second = validateInput(secondThrow);
    ...
    public int getBothThrows() {
        if(this.secondThrow == '/') return 10;
        else if(this.firstThrow == 'X') return 11; //For double strike case, handled in Game class
