...
public class Groups extends AppCompatActivity {
    ...
    Groups(ArrayList<String> members) {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                if (dataSnapshot.child("groups").hasChild(groupName)) {
                    ...
                    if (dataSnapshot.child("coaches").hasChild(username)) {
                        myRef.child(groupName).child(currUsernameText.getText().toString()).setValue("2");
                        create(groupName);
                    }
                    else {
                        myRef.child(groupName).child(currUsernameText.getText().toString()).setValue("1");
                        create(groupName);
                    }
                }
            }
